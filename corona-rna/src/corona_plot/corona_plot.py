...
def plot_matrix_pc_(matrix, pcs, title, r, types = 'nomal'):
    '''
    ...
    if types == 'nomal' :
        sc = ax.imshow(matrix, cmap = plt.get_cmap('bwr'), vmax = vmax, vmin = 0-vmax,aspect = 'auto', interpolation = 'none', 
                   origin = 'lower',extent = (0, lenth, 0, lenth))
    elif types == 'oe' :
    ...
    for i in range(lens):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_ylabel(title[i], fontsize=10, rotation = 'horizontal', labelpad = 40)
...
def plot_matrix_ct(matrix, pcs, ct, title, s , e, r):
    ...
    for i in range(lens):
        ax = fig.add_axes([left, bottom_h, width, stept_h])
        ax.fill_between(np.arange(len(pcs[i])), pcs[i], color = 'grey')
        ax.set_xlim((0,len(pcs[i])-1))
        ax.set_xticks([])
        ax.set_yticks([])
...
def joint_heatmap_hist(matrix, title, r, vmax, cmap, types = 'nomal'):
    '''
    plot a compartment and heatmap mix fig
    '''
    ##ax size
    ...
    bottom, height = 0.1, 0.65
    size_heatmap = [left, bottom, width, height]
    size_colorbar = [0.5, 0.5, 0.005, 0.15]
    ...
    lenth = shp[0] * r 
    ma = np.log2(matrix+1)
    sc = ax.imshow(np.triu(ma).T, cmap = plt.get_cmap(cmap), vmax = vmax, vmin = 0
    ...
    ##compartments
    hist_data = np.triu(matrix).sum(axis = 1)
    hist_lens = hist_data.shape[0]
    stept_h = 0.15
    bottom_h = bottom + height
    ax = fig.add_axes([left, bottom_h, width, stept_h])
    plt.plot(hist_data, color='grey', lw = 0.15)
    ax.set_xlim((0,hist_lens - 1))
    ax.set_xticks([])
 #   ax.set_yticks([])
    ax.set_ylabel('Counts', fontsize=10, labelpad = 10)# rotation = 'horizontal'
    ax.spines[['top','right','left','bottom']].set_visible(False)
    bottom_h = bottom_h + stept_h
    
    ##
    hist_data = np.triu(matrix).sum(axis = 0)
    stept_w = 0.15
    bottom_w = left + width 
    ax = fig.add_axes([bottom_w, bottom, stept_w,  height])
    plt.plot(hist_data[::-1], np.arange(hist_lens), color='grey', lw = 0.15)
    ax.set_ylim((0,hist_lens-1))
  #  ax.set_xticks([])
    ax.set_yticks([])
    ax.set_xlabel('Counts', fontsize=10, labelpad = 10)# rotation = 'horizontal'
    ax.spines[['top','right','left','bottom']].set_visible(False) 
    bottom_w = bottom_w + stept_w
    
    return fig

def joint_heatmap_hist_genome(matrix, dict_gene, title, r, vmax, cmap):
    '''
    plot a compartment and heatmap mix fig
    '''
    ##ax size
    ...
    bottom, height = 0.1, 0.65
    size_heatmap = [left, bottom, width, height]
    size_colorbar = [0.5, 0.5, 0.025, 0.15]
    n = int(50 / r)
    fig = plt.figure(figsize=(10,10))
    ...
    lenth = shp[0] * r 
    ma = np.log2(matrix+1)
    ma[np.triu_indices_from(ma)] = np.nan
    sc = ax.imshow(ma, cmap = plt.get_cmap(cmap), vmax = vmax, vmin = 0
              , aspect = 'auto',extent = (0, lenth, lenth, 0))
    ax.spines[['top','right']].set_visible(False)    
    ax.set_xlabel("5' junction sites (Bp)",fontsize=15)
    ax.set_ylabel("3' junction sites (Bp)",fontsize=15)
    ax = fig.add_axes(size_colorbar)
    fig.colorbar(sc,cax = ax)
 ...
 #    ## genome
 #    ax = fig.add_axes([left, bottom_h + 0.01, width, 0.05])
    ...
    ##junc sites
    hist_data = np.triu(matrix, 2).sum(axis = 0)
    stept_w = 0.1
    bottom_w = left + width 
    ax = fig.add_axes([bottom_w, bottom, stept_w,  height])
    plt.plot(np.log2(hist_data[::-1]), np.arange(hist_lens), color='grey', lw = 0.15)
    ax.set_ylim((0,hist_lens-1))
  #  ax.set_xticks([])
    ax.set_yticks([])
    ax.set_xlabel('Counts', fontsize=10, labelpad = 10)# rotation = 'horizontal'
    ...
    ax = fig.add_axes([bottom_w+0.01, bottom, 0.05,  height])
    i = 0
    for key in dict_gene:
        s,e = dict_gene[key]
        s = s / r
